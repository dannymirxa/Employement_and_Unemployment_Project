...
```{r}
clean_df <- df
# Find the number of NAs in each column of data frame
print(sapply(clean_df, function(x) sum(is.na(x)))) 

# Impute NAs using linear regression model
i<-1
j<-1
for (i in 1:nrow(clean_df)){
     for (j in 1:ncol(clean_df)){
         if (is.na(clean_df[i,j])==TRUE){
             ...
             new_data<-clean_df[(i-2):(i+2),"year"]
...
data <- read.csv("D:\\OneDrive\\Project\\Employment_and_Unemployment_Project\\Data")
data
```
